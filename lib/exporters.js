...
import { blankIfZeroHours, hasOrangeWork, minsToHHMM, parseHoursToMinutes } from './timesheet';
  ...
  const tables = weeks.map((weekDays, weekIdx) => {
    ...
    const extraCells = weekDays
      .map((info) => {
        ...
        const entry = entryFor(entries, year, month, info.day);
        const value = entry ? blankIfZeroHours(entry.orange_hours) : '';
    ...
    weeks.forEach((weekDays, weekIdx) => {
      ...
      const data = [
        ...
        ['extra hrs', ...weekDays.map((info) => {
          if (info.isSun) return 'X';
          const entry = entryFor(entries, year, month, info.day);
          return entry ? blankIfZeroHours(entry.orange_hours) : '';
...
function buildHoursSummarySheet({ summary, monthName }) {
  return [
    ['MONTHLY HOURS'],
    [monthName],
    [],
    ['Work #', 'Name', 'White Paper Hrs', 'Orange Paper Hrs', 'Green Paper Hrs', 'Total Hrs'],
    ...summary.map((r) => [r.work_number, r.full_name, r.white_hours, r.orange_hours, r.green_hours, r.total_hours]),
    [],
    [
      'TOTAL',
      '',
      minsToHHMM(summary.reduce((s, r) => s + parseHoursToMinutes(r.white_hours), 0)),
      minsToHHMM(summary.reduce((s, r) => s + parseHoursToMinutes(r.orange_hours), 0)),
      minsToHHMM(summary.reduce((s, r) => s + parseHoursToMinutes(r.green_hours), 0)),
      minsToHHMM(summary.reduce((s, r) => s + parseHoursToMinutes(r.total_hours), 0)),
    ],
  ];
}

export async function exportHoursSummaryExcel({ summary, month, year }) {
  const monthName = `${MONTH_NAMES[month - 1]} ${year}`;
  const wb = XLSX.utils.book_new();
  const data = buildHoursSummarySheet({ summary, monthName });
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(data), 'Monthly Hours');
  const base64 = XLSX.write(wb, { type: 'base64', bookType: 'xlsx' });
  const filename = `monthly-hours-${month}-${year}.xlsx`;
  const uri = `${FileSystem.cacheDirectory}${filename}`;
  await FileSystem.writeAsStringAsync(uri, base64, { encoding: FileSystem.EncodingType.Base64 });
  await Sharing.shareAsync(uri, {
    mimeType: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
