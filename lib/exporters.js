...
import { minsToHHMM, parseHoursToMinutes } from './timesheet';
...
function buildOrangeHtml({ worker, monthName, daysCount, entries, year, month }) {
  const rows = Array.from({ length: daysCount }, (_, i) => {
    const day = i + 1;
    const entry = entryFor(entries, year, month, day);
    const bg = entry ? '#fff8e1' : '#fffbf0';
    return `<tr style="background:${bg}">
      <td style="border:0.5pt solid #c97d00"><b>${day}</b></td>
      <td style="border:0.5pt solid #c97d00">${escapeHtml(entry?.orange_start?.slice(0, 5))}</td>
      <td style="border:0.5pt solid #c97d00">${escapeHtml(entry?.orange_finish?.slice(0, 5))}</td>
      <td style="border:0.5pt solid #c97d00">${escapeHtml(entry ? entry.orange_break || '0:00' : '')}</td>
      <td style="border:0.5pt solid #c97d00">${escapeHtml(entry?.orange_hours)}</td>
      <td style="border:0.5pt solid #c97d00; text-align:left">${escapeHtml(entry?.what_work)}</td>
...
function buildWeeklyHtml({ worker, monthName, daysCount, entries, year, month }) {
  ...
  const tables = weeks.map((weekDays, weekIdx) => {
    ...
    const pickupCells = weekDays
      .map((info) => `<td style="border:0.5pt solid #333; background:#e8f5e9; color:#2d6a2d; font-weight:700">${info.isSun ? 'X' : ''}</td>`)
      .join('');
    ...
    return `<p style="font-weight:800; font-size:11px; text-transform:uppercase; margin-top:14px">Week ${weekIdx + 1}</p>
      <table>
        ...
        <tbody>
          <tr>
            <td style="border:0.5pt solid #333; background:#e8f5e9; color:#2d6a2d; font-weight:700; text-align:left">pickup hours</td>
            ${pickupCells}
            <td style="border:0.5pt solid #333; background:#e8f5e9; color:#2d6a2d; font-weight:700"></td>
          </tr>
          <tr>
...
function buildGreenHtml({ worker, monthName, daysCount, entries, year, month }) {
  const rows = Array.from({ length: daysCount }, (_, i) => {
    const day = i + 1;
    const entry = entryFor(entries, year, month, day);
    const bg = entry ? '#f6fff6' : '#ffffff';
    return `<tr style="background:${bg}">
      <td style="border:0.5pt solid #2d6a2d"><b>${day}</b></td>
      <td style="border:0.5pt solid #2d6a2d">${escapeHtml(entry?.actual_start?.slice(0, 5))}</td>
      <td style="border:0.5pt solid #2d6a2d">${escapeHtml(entry?.actual_finish?.slice(0, 5))}</td>
      <td style="border:0.5pt solid #2d6a2d; color:#888888">1 hour</td>
      <td style="border:0.5pt solid #2d6a2d">${escapeHtml(entry?.orange_break && entry.orange_break !== '0:00' ? entry.orange_break : '')}</td>
      <td style="border:0.5pt solid #2d6a2d"><b style="color:#2d6a2d">${escapeHtml(entry?.white_hours)}</b></td>
      <td style="border:0.5pt solid #2d6a2d; text-align:left">${escapeHtml(entry?.what_work)}</td>
      <td style="border:0.5pt solid #2d6a2d">${entry?.kg_picked != null ? `<b style="color:#2d6a2d">${escapeHtml(entry.kg_picked)}</b>` : ''}</td>
    </tr>`;
  }).join('');
...
export async function exportPaperPdf(tab, { month, year, worker, entries, daysInMonth }) {
  const monthName = `${MONTH_NAMES[month - 1]} ${year}`;
  const html = PDF_BUILDERS[tab]({ worker, monthName, daysCount: daysInMonth, entries, year, month });
...
function buildOrangeSheet({ worker, monthName, daysCount, entries, year, month }) {
  return [
    ...
    ...Array.from({ length: daysCount }, (_, i) => {
      ...
      return [
        day,
        entry?.orange_start?.slice(0, 5) || '',
        entry?.orange_finish?.slice(0, 5) || '',
        entry ? entry.orange_break || '0:00' : '',
        entry?.orange_hours || '',
        entry?.what_work || '',
        '',
      ];
...
function buildGreenSheet({ worker, monthName, daysCount, entries, year, month }) {
  return [
    ...
    ...Array.from({ length: daysCount }, (_, i) => {
      ...
      return [
        day,
        entry?.actual_start?.slice(0, 5) || '',
        entry?.actual_finish?.slice(0, 5) || '',
        '1 hour',
        entry?.orange_break && entry.orange_break !== '0:00' ? entry.orange_break : '',
        entry?.white_hours || '',
        entry?.what_work || '',
        entry?.kg_picked != null ? entry.kg_picked : '',
      ];
    }),
  ];
}

export async function exportPaperExcel(tab, { month, year, worker, entries, daysInMonth }) {
  const monthName = `${MONTH_NAMES[month - 1]} ${year}`;
  ...
  if (tab === 'white') {
    ...
    weeks.forEach((weekDays, weekIdx) => {
      ...
      const data = [
        ...
        ['pickup hours', ...weekDays.map((info) => (info.isSun ? 'X' : '')), ''],
    ...
    const data = buildGreenSheet({ worker, monthName, daysCount: daysInMonth, entries, year, month });
