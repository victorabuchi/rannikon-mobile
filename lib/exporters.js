...
function buildGreenHtml({ worker, monthName, daysCount }) {
  const rows = Array.from({ length: daysCount }, (_, i) => {
    const day = i + 1;
...
function buildGreenSheet({ worker, monthName, daysCount }) {
  return [
...
export async function exportPaperExcel(tab, { month, year, worker, entries, daysInMonth }) {
  ...
  if (tab === 'white') {
    ...
    const data = buildGreenSheet({ worker, monthName, daysCount: daysInMonth });
